...
class CombinatorialEnumerate(object):
    ...
    def __init__(self, tao=0.2, gamma=0.7, beta=0.2, res=0.2, \
...
class Random(object):
    ...
    def __init__(self, tao=0.2, gamma=0.7, beta=0.2, res=0.2,\
        ...
        if self.augment_threshold == -1:
            ...
            if self.augment_type_for_short == 'SI':
            ...
            elif self.augment_type_for_short == 'SIMRC':
                self.short_seq_data_aug_methods = [Insert(item_similarity_model, insert_rate=insert_rate,
                                                          max_insert_num_per_pos=max_insert_num_per_pos,
                                                          augment_threshold=self.augment_threshold),
                                                   Substitute(item_similarity_model, substitute_rate=substitute_rate),
                                                   Reorder(beta=gamma), Crop(tao=tao), Mask(gamma=gamma)]
            else:
                ...
                self.short_seq_data_aug_methods = [Insert(item_similarity_model, insert_rate=insert_rate,
                                                          ...
                                                          augment_threshold=self.augment_threshold),
                                                   Substitute(item_similarity_model, substitute_rate=substitute_rate),
                                                   Crop(tao=tao), Mask(gamma=gamma), Reorder(beta=gamma), Reserve(res=res)]
...
#增加插入操作（相似项目，其中近期偏好的项目权重较大，考虑时序因素），除了相似项目考虑将相似用户的item插入
class Insert(object):
...
class Substitute(object):
    ...
    def __call__(self, sequence):
        # make a deep copy to avoid original sequence be modified
        copied_sequence = copy.deepcopy(sequence)   #原始序列
        substitute_nums = max(int(self.substitute_rate * len(copied_sequence)), 1)  #替换的个数
        substitute_idx = random.sample([i for i in range(len(copied_sequence))], k=substitute_nums)  #随机替换的索引
        inserted_sequence = []
...
class Reserve(object):
    """conduct the new sentence with reserve sub seq"""
    ...
    def __call__(self, sequence):
        # make a deep copy to avoid original sequence be modified
        copied_sequence = copy.deepcopy(sequence)
        sub_seq_length = int(self.res * len(copied_sequence))
        start_index = 0
        sub_seq = copied_sequence[start_index : start_index + sub_seq_length]
        # print(sub_seq)
        sub_seq_res = list(reversed(sub_seq))
        # print(sub_seq_res)
        reserved_seq = copied_sequence + sub_seq_res
        return reserved_seq
